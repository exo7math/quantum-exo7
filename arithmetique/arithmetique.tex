...
Le test de Fermat permet de produire des entiers très grands qui sont probablement des nombres premiers : on choisit un entier $n$ au hasard, on effectue un test de Fermat, si le test est concluant alors $n$ est probablement un nombre premier, sinon on essaie l'entier $n+1$\ldots
...
Le meilleur algorithme connu pour factoriser des grands entiers est l'algorithme GNFS (pour \emph{General Number Field Sieve}), sa complexité est environ $O(n^{\frac13})$.
